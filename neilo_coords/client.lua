...
local function copyCoords(coords)
    ...
    -- print to chat
    TriggerEvent('chat:addMessage', { args = { '^2[NeiloCoords] ^7Coordinates copied:', text } })
end
...
RegisterCommand('toggleball', function()
    ...
    if not enabled then
        removeBall()
        TriggerEvent('chat:addMessage', { args = { '^5[NeiloCoords] ^7Disabled.' } })
    else
        TriggerEvent('chat:addMessage', { args = { '^5[NeiloCoords] ^7Enabled. Use F7 to copy coordinates.' } })
    end
end, false)
...
RegisterCommand('copycoords', function()
    if lastCoords then
        ...
        TriggerEvent('chat:addMessage', { args = { '^5[NeiloCoords] ^7No valid coordinates found.' } })
    end
end, false)
